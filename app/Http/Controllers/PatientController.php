...
//        $this->middleware('permission:org-view', ['only' => ['show']]);
    ...
    public function show($id)
    {
        ...
        $cmss2 = CMS::with('patient', 'patient.physician')->get();
        $physicianOrders2 = PhysicianOrders::with('patient', 'patient.physician')->get();

        $data = [];

        foreach ($cmss2 as $cms) {
            $data[] = [
                'type' => 'CMS',
                'id' => $cms->id,
                'patient_fname' => $order->patient->first_name ?? 'N/A',
                'patient_lname' => $order->patient->last_name ?? 'N/A',
                ...
                'pdf_path' => $cms->pdf_path ?? '',
            ];
        }

        foreach ($physicianOrders2 as $order) {
            $data[] = [
