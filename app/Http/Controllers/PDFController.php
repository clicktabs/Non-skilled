...
class PDFController extends Controller
{
    public function generatePDF(Request $request, $id)
    {
        ...
        $physician = PhysicianOrder::where('patient_history_id', $id)->first();
        $patient = Patient::where('id', $id)->first();
        $episodeDaterange = PatientEpisodeManager::select('episode_start_date', 'episode_end_date')->where('id',$id)->first();
        $medications = Medication::select('medication_dosage','frequency','route','status')->where('patient_id', $id)->get();

        // $html = View::make('pdf.home-care.aide-supervisory', compact('request'))->render();
        $html = View::make('pdf.home-care.oasis-e-start-of-care', compact('request','administrative','demographic','hearingVision','cognitiveMoodBehavior','preference','functionalSts','functionalAbl','bladderBowel','actDiagnosis','healthCon','swallNutritional','skinCondition','medication','specialTrmnt','physician', 'patient', 'episodeDaterange', 'medications'))->render();
    ...
    public function PatientEmergencyContact(Request $request) {
        $contacts = PatientEmergencyContact::findMany( $request->contact_ids );
        $reportType = $request->reportType;

        $html = View::make('pdf.patients.emergency-contact-pdf', compact('contacts', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_emergency-contact_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
    ...
    public function PatientPhysician(Request $request) {
        $physicians = Physician::findMany( $request->physician_ids );
        $reportType = $request->reportType;

        $html = View::make('pdf.patients.physicians-pdf', compact('physicians', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_patient-physicians_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
    ...
    public function PatientStartOfCare(Request $request) {
        ...
        $html = View::make('pdf.patients.start-of-care-pdf', compact('episodes', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_patient-start-of-care_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
    ...
    public function PatientPlanOfCare(Request $request) {
        $orders = PhysicianOrder::findMany( $request->order_ids );
        $reportType = $request->reportType;

        $html = View::make('pdf.patients.plan-of-care-pdf', compact('orders', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_patient-plan-of-care_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
    ...
    public function PatientDischargeList(Request $request) {
        $discharges = PatientDischarge::findMany( $request->discharge_ids );
        $reportType = $request->reportType;

        $html = View::make('pdf.patients.discharge-list-pdf', compact('discharges', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_patient-discharge-list_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
    ...
    public function PatientInfectionList(Request $request) {
        $infections = InfectionControl::findMany( $request->infection_ids );
        $reportType = $request->reportType;

        $html = View::make('pdf.patients.infection-list-pdf', compact('infections', 'reportType'))->render();

        $dompdf = new Dompdf();
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'potrait');
        $dompdf->render();
        $filename = 'pdf_patient-infection-list_' . time() . '.pdf';
        $filePath = 'pdfs/' . $filename;
        Storage::put($filePath, $dompdf->output());
