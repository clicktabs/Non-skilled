    ...
    $(function () {
        ...
        */
        ...
        function patientRosterTable() {
            $('#PatientRosterTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
        ...
        function patientEmergencyContact() {    
            $('#PatientEmergencyContactTable').show().DataTable({
                ...
                destroy: true,
                ajax : {
                    url : "{{ route('patients.patient-emergency-contact.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
        ...
        // Patient Physician List
        function patientPhysicianList() {
            $('#PatientphysicianListTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
                    url : "{{ route('patients.physician.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
                    {data : 'patient', name : 'patient'},
        ...
        // Patient Physician List
        function patientStartofCareList() {
            $('#PatientStartofCareListTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
                    url : "{{ route('patients.startofcare.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
                    {data : 'name', name : 'name'},
        ...
        // Plan of Care History
        function planofCareHistory() {
            $('#PlanofCareHistoryTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
                    url : "{{ route('patients.plan-of-care.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
                    ...
                    {data : 'mrn', name : 'mrn'},
                    {data : 'dob', name : 'dob'},
                    {data : 'care_date', name : 'care_date'},
                    {data : 'physician_name', name : 'physician_name'},
                    {data : 'physician_npi', name : 'physician_npi'},
                ]
            });
        }
        ...
        function patientDischargeList() {
            $('#PatientDischargeListTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
                    url : "{{ route('patients.discharge-list.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
                    {data : 'patient_name', name : 'patient_name'},
        ...
        // Patient Infection List
        function patientInfectionList() {
            $('#PatientInfectionListTable').show().DataTable({
                processing : true,
                serverSide : true,
                destroy: true,
                ajax : {
                    url : "{{ route('patients.infection-list.ajax') }}",
                },
                columns : [
                    {data : 'id', name : 'id'},
                    {data : 'patient_name', name : 'patient_name'},
        ...
        // Show Table when select Report Type
        $('#switch_table_data').on('change', function() {
            ...
            let formId = getType.replace(/ /g, "") + 'Form';
            let generateButton = $('#generateButton');
            generateButton.attr('form', formId);

            // Get Type of Report from Selection
            let reportType = $(this).find('option:selected').text();
            $('#'+formId).find('.report-type').val(reportType);
            ...
            switch(getType) {
                ...
                case 'Patient Start of Care List':
                    patientStartofCareList();
                    break;

                case 'Plan of Care History':
                    planofCareHistory();
                    break;

                case 'Patient Discharge List':
