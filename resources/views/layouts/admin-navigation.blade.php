...
if ($routeName != 'dashboard') {
    ...
    if (!empty($user->organization_id)) {
        $subscription = App\Models\Subscription::where('organization_id', $user->organization_id)->where('status', 'active')->first();

        if($subscription){
            $subscriptionTypeName = App\Models\SubscriptionPlan::where('id',$subscription->subscription_plan_id)->first('type');
...
<div>
    ...
    <!-- Sidebar -->
    <div id="sidebar" class="flex ak g tb x k tea tec teh tts ss lp tth l or tej ttz 2xl:!w-64 ub hs dw we wr wu !bg-[#2c3e75]"
        :class="sidebarOpen ? 'translate-x-0' : '-translate-x-64'" @click.outside="sidebarOpen = false"
        @keydown.escape.window="sidebarOpen = false" x-cloak="lg">

        <!-- Sidebar header -->
        <h1 class="text-white" style="margin-left: 10px;">Non-skilled Platform</h1>
        ...
        <div class="ff rounded-[10px] bg-[#fff]">
            <!-- Pages group -->
            @if (isset($subscriptionPlanType) && $subscriptionPlanType == 'Home healthcare')
            <div>
                <ul class="upper">
                    <!-- Dashboard -->
                    <li
            ...
            <div>
                ...
                <ul class="nk">
                    <!-- Setups -->
                    @if (isset($subscriptionPlanType) && $subscriptionPlanType)
