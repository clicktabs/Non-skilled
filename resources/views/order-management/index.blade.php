                                ...
                                <tbody class="  text-sm gp le ln">
                                    @forelse ($data as $key => $item)
                                        <tr class="active cursor-pointer ">
                                            ...
                                            <td class="">
                                                <div><input type="file" class="qa-pdf-date-input" accept=".pdf" data-item-id="{{ $item['id'] }}" data-item-type="{{ $item['type'] }}"></div>
                                            </td>
...
<script>
    $(document).ready(function () {
        $('.qa-pdf-date-input').change(function () {
            var itemId = $(this).data('item-id');
            var itemType = $(this).data('item-type');
            var selectedPdfFile = $(this)[0].files[0];

            var formData = new FormData();
            formData.append('itemId', itemId);
            formData.append('itemType', itemType);
            formData.append('selectedpdf', selectedPdfFile);

            $.ajax({
                url: "{{ route('save.pdf.date') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
