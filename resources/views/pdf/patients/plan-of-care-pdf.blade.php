            ...
            @if ($orders)
                ...
                @foreach ($orders as $key => $order)
                    {{++$key}}
                        ...
                        <td>
                            <p>{{$order->id}}. <b>Patient Name:</b> {{$order->patientName}} ({{$order->physicianGender=='female' ? 'F' : 'M'}})</p>

                            <p><b>MRN#</b> {{$order->patient->patient_code}}</p>

                            @if ($order->patient->date_of_birth)
                                <p><b>Date of Birth:</b> {{$order->patient->date_of_birth}}</p>
                            @endif

                            @if ($order->created_at)
                                <p><b>Plan of Care Date:</b> {{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $order->created_at)->format('Y-m-d')}}</p>
                            @endif

                            @if ($order->physicianName)
                                <p><b>Physician Name:</b> {{$order->physicianName}}</p>
                            @endif

                            @if ($order->physician->npi_number)
                                <p><b>Physician NPI:</b> {{$order->physician->npi_number}}</p>
                            @endif
                        </td>
