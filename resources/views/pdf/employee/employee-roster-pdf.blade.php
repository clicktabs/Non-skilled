        ...
        <table class="table">
            @if ($employees)
                ...
                @foreach ($employees as $key => $employee)
                    ...
                    {{-- @php
                        $address = App\Models\PatientAddressInfo::where('patient_id', $employee->id)->first();
                    @endphp --}}
                    
                        <td>
                            <p><b>Employee {{$employee->employee_id}}</b> | <u>{{$employee->mobile_phone}}</u></p>
                            <p><b>{{$employee->id . '. ' . $employee->first_name . ' ' . $employee->last_name }} ({{$employee->gender=='female' ? 'F' : 'M'}})</b></p>

                            @if ($employee->dob)
                                <p>DOB: {{\Carbon\Carbon::parse($employee->dob)->format('M d, Y')}} | Age: {{\Carbon\Carbon::parse($employee->dob)->age}}</p> 
                            @endif
                            
                            @if ($employee)
                                <p>{{$employee->address_line_1}}</p>
                            @endif
                        </td>
